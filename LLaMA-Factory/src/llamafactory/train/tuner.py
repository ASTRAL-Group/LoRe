...
def run_exp(args: Optional[dict[str, Any]] = None, callbacks: Optional[list["TrainerCallback"]] = None) -> None:
    ...
    if args.get("output_dir") is None or args["output_dir"] == "":
        model = args["model_name_or_path"].split("/")[-1].lower().replace("_", "").replace(".", "")
        dataset = args["dataset"].lower().replace("_", "").replace(".", "")
        bs = args["per_device_train_batch_size"]
        epoch = args["num_train_epochs"]
        ga = args["gradient_accumulation_steps"]
        lr = args["learning_rate"]
        wr = args.get("warmup_ratio", 0)
        ctx = args["cutoff_len"]
        n_gpus = torch.cuda.device_count()

        tag = f"{model}-{dataset}-bs{bs}-epoch{epoch}-ctx{ctx}-ga{ga}-lr{lr}-wr{wr}-n{n_gpus}"
        args["output_dir"] = f".../LLaMA-Factory/saves/{tag}"
        args["run_name"] = tag
